...
export function isExpired(createdAt, expiresInDays) {
 return new Date(Date.now()) - new Date(createdAt) > 24 * 60 * 60 * 1000 * expiresInDays;
}
...
export const capturePaymentController = async (req, res) => {
  try {
    const {
      razorpay_order_id,
      razorpay_payment_id,
      razorpay_signature, amount} = req.body;
      const userId = req.user._id;
      await capturePaymentService(razorpay_order_id, razorpay_payment_id, razorpay_signature, amount/100, userId);
      res.status(StatusCodes.OK).json(customSuccessResponse("Payment captured successfully", {}));
  }
  catch (err) {
    console.log(err)
    if (err.statusCode) {
