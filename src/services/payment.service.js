...
export const capturePaymentService = async (orderId, paymentId, signature, amount, userId) => {
  ...
  if (generated_signature !== signature) {
    ...
    let paymentData;
    if (alreadyCaptured) {
        paymentData = await paymentRepo.update(alreadyCaptured._id, {orderId, paymentId, signature, amount});
    }
    else {
      paymentData  = await paymentRepo.create({orderId, paymentId, signature, amount, userId});
    }
